  ...
  <header class="w-full flex flex justify-between items-center py-6 px-8 text-white bg-primary">
    ...
    <nav>
      <ul class="flex">
        ...
        <span class="absolute top-0 right-2 -mt-2 bg-red-500 rounded-full w-5 h-5 text-white text-xs flex items-center justify-center">
          {totalItemsFromCart}
        </span>
        {#if isCartVisible}
          <div class="triangle absolute top-8 left-6 bg-white shadow-md"></div>
          <div class="fixed absolute top-11 right-0 w-96 z-50 rounded grid bg-white drop-shadow-lg"
            on:mouseenter={handleCartMouseEnter}
            on:mouseleave={handleCartMouseLeave} 
          >
            {#if $cart.length === 0}
